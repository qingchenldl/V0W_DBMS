...
class BTree(object):
    ...
    def __RL(self,r):#右左，先右旋再左旋
        ...
        return self.__RR(r)
    def __insert(self,data,r,row,col):
        if r==None:
            node=TreeNode()
            node.data=data
            node.row=row
            ...
            r.left=self.__insert(data,r.left,row,col)
            ...
            r.right=self.__insert(data,r.right,row,col)
    ...
    def __delete(self,data,r):
        ...
        r.height=self.__Max(self.getHeight(r.left),self.getHeight(r.right))+1
        return r
    def __search(self,data,r):
        if r==None:
            return -1
        elif data==r.data:
            return [data,r.row,r.col]
        elif data<r.data:
            self.__search(data,r.left)
        else:
            self.__search(data,r.right)
    #先序遍历
    ...
    def Insert(self,data,row,col):
        self.root=self.__insert(data,self.root,row,col)
        return self.root
